...
> **🔥 현재 최우선(2026-07-12): E-BOOK 만료 링크 정리(ROADMAP §5-2 = §0 U4).** 단, 48개 전수 점검서 하드 다운(404) 0건 — "소프트 로트"(200인데 특정 책만 내려감)만 남아 응급은 아니며, 확인·교체는 사용자 몫. 나머지 최우선 2개(성적 산출기 v2·계산 단위 테스트)는 ✅ 완료. 상세·다음 순위는 ROADMAP §5(**2026-07-14에 3개월 실행 로드맵으로 갱신** — §4·§7과 충돌 시 §5 우선).
...
1. **모든 수정 후 `node tools/verify.mjs`** — 5항목(import 정합성/문법/그래프 로드+init/SW 프리캐시 존재/**[5] 계산 단위 테스트 38건**) 통과 못 하면 배포 금지. 새 DOM API를 쓰면 verify.mjs의 DOM 스텁에도 추가. **계산 로직(gradecalc·chasi·evalplan)을 고치면 `tools/test.mjs`에 케이스를 먼저/함께 추가**할 것 — 실무 사고 방지선. 테스트용 export는 `__gcTest`/`__chasiTest`/`__evalTest`(앱은 render 함수만 씀). chasi는 Date+toISOString TZ 시프트가 있어 테스트는 TZ 불변값(총차시·주당시수)만 단언.
...
| `sw-life` | 소프트웨어와 생활 5단원 시리즈 (데과의 형제, 웜 무드) | **라이브** jungbucks.github.io/sw-life (07-15 확인) |
| `sort-lab` | 한눈에 보는 정렬 실험실 (카드 5장 스텝 시각화) | **라이브** jungbucks.github.io/sort-lab (07-15 확인) |
| `cipher` | 암호의 세계 — 암호학 시각화 단일 HTML (6탭·브루트포스·해시) | 로컬 커밋 완료·**GitHub 저장소 생성 대기**(리모트 설정됨), 앱스토어 v29 등록 |
