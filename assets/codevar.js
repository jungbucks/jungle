import { esc, clipboardWriteText } from './utils.js';
...
function renderCodeVar() {
  const savedKey = (() => { try { return localStorage.getItem('gemini_api_key') || ''; } catch(e) { return ''; } })();
  ...
  return `<div class="codevar-wrap">
    <div class="eval-settings">
      <div class="codevar-intro">
        <div class="codevar-intro-title">코드 변형 생성기</div>
        <div class="codevar-intro-sub" id="codevarIntroSub">${isVar
      ...
      <div class="eval-settings-row">
        <div class="eval-field">
          ...
          <div class="codevar-lang-group">
            <button class="codevar-lang-btn${codevarLang === 'Python' ? ' active' : ''}" onclick="codevarSetLang('Python')">Python</button>
            <button class="codevar-lang-btn${codevarLang === 'C' ? ' active' : ''}" onclick="codevarSetLang('C')">C</button>
          </div>
        </div>
        <div class="eval-field">
          <span class="eval-label">모드</span>
          <div class="codevar-lang-group">
            <button class="codevar-mode-btn codevar-lang-btn${isVar ? ' active' : ''}" data-mode="variation" onclick="codevarSetMode('variation')">코드 변형</button>
            <button class="codevar-mode-btn codevar-lang-btn${!isVar ? ' active' : ''}" data-mode="problem" onclick="codevarSetMode('problem')">문제 생성</button>
          </div>
        </div>
        <div class="eval-field" style="flex:1;min-width:200px">
          ...
          <em class="codevar-apikey-privacy">API 키를 추가해 사용하셔도 '정글' 사이트에는 해당 정보가 전송되지 않습니다.</em>
        </div>
      </div>
      <div class="eval-settings-row">
      ...
      <div class="eval-settings-row" style="align-items:center;margin-bottom:0">
        <button id="codevarBtn" class="codevar-gen-btn" onclick="codevarGenerate()">${isVar ? '✨ 변형 생성' : '✨ 문제 생성'}</button>
    ...
    return `<div class="codevar-card">
      ...
      <div class="codevar-code-wrap">
        ...
        <div class="codevar-code-hd">
          ...
          <button class="codevar-copy-btn" onclick="codevarCopy(this,${i})">복사</button>
        </div>
...
window.codevarSetLang = codevarSetLang;
window.codevarSetMode = codevarSetMode;
window.codevarGenerate = codevarGenerate;
window.codevarCopy = codevarCopy;
