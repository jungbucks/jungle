import { esc, loadState } from './utils.js';
...
// --- Chasi Calculator ---
function chasiSave() { localStorage.setItem('jungle_chasi', JSON.stringify(chasiState)); }
...
function chasiToast(msg, isErr = false) {  
  const t = document.createElement('div');  
  t.textContent = msg;  
  t.style.cssText = `position:fixed;bottom:80px;left:50%;transform:translateX(-50%);    
    background:${isErr?'#ef4444':'#0891B2'};color:#fff;padding:10px 20px;border-radius:8px;    
    font-size:14px;font-weight:500;z-index:9999;white-space:nowrap;box-shadow:0 4px 12px rgba(0,0,0,.18)`;  
...
function renderChasi() {  
  ...
  const accent = '#0891B2', aLight = '#ECFEFF', aDark = '#0E7490';  
  ...
  return `<div class="chasi-wrap">    
    <div class="chasi-section">      
      <div class="chasi-section-title" style="color:${aDark}">📅 기본 설정</div>      
      <div class="chasi-row">        
        <div class="chasi-field">          
          <label class="chasi-label">학년도</label>          
          <select class="chasi-select" onchange="chasiSetYear(this.value)">            
        ...
        <div class="chasi-field">          
          ...
          <div class="chasi-seg">            
            <button class="chasi-seg-btn${cs.semester===1?' active':''}" onclick="chasiSetSemester(1)" aria-pressed="${cs.semester===1}"
              style="${cs.semester===1?`background:${accent};color:#fff;border-color:${accent}`:''}">1학기</button>
            <button class="chasi-seg-btn${cs.semester===2?' active':''}" onclick="chasiSetSemester(2)" aria-pressed="${cs.semester===2}"
      ...
      <div class="chasi-row">        
        <div class="chasi-field">          
          ...
          <input class="chasi-input" type="date" value="${cs.startDate}" onchange="chasiSetStart(this.value)">        
        </div>        
        <div class="chasi-field">          
          <label class="chasi-label">종료일</label>          
          <input class="chasi-input" type="date" value="${cs.endDate}" onchange="chasiSetEnd(this.value)">        
        </div>      
      </div>      
      <div class="chasi-field">        
        <div class="chasi-label-row">          
          <label class="chasi-label">수업 요일</label>          
          <button class="chasi-block-btn${cs.blockTime?' active':''}" onclick="chasiToggleBlockTime()">            
            블록타임 (2시수)          
          </button>        
        </div>        
        <div class="chasi-days-row">          
          ${DAYS.map((day, i) => {            
            ...
            const on = hrs > 0;            
            return `<button class="chasi-day-btn${on?' active':''}" onclick="chasiToggleDay(${i})" aria-pressed="${on}"
        ...
        : `<div class="chasi-exc-list">            
            ${excSorted.map(e => `              
              <div class="chasi-exc-item">                
                ...
                <button class="chasi-exc-del" onclick="chasiRemoveException(${e.id})" aria-label="삭제">✕</button>              
              </div>`).join('')}          
          </div>`}    
    </div>    
    <div class="chasi-section">      
      <div class="chasi-section-title" style="color:${aDark}">📊 계산 결과</div>      
      <div class="chasi-result-total" style="background:${aLight};border-color:${accent}">        
        ...
        ${weekAlerts.length>0?`<span style="margin-left:${wh>0?'8':'auto'}px;font-size:12px;color:#f59e0b;font-weight:600">⚠ ${weekAlerts.length}주 변동</span>`:''}      
      </div>      
      ${monthly.length>0?`      
...
export { renderChasi };
window.chasiSave = chasiSave;
window.chasiSetYear = chasiSetYear;
window.chasiSetSemester = chasiSetSemester;
window.chasiSetStart = chasiSetStart;
window.chasiSetEnd = chasiSetEnd;
window.chasiToggleDay = chasiToggleDay;
window.chasiToggleBlockTime = chasiToggleBlockTime;
window.chasiExcSetField = chasiExcSetField;
window.chasiAddException = chasiAddException;
window.chasiLoadHolidays = chasiLoadHolidays;
window.chasiRemoveException = chasiRemoveException;
