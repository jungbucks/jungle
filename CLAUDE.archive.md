...
7. ~~(선택) CSP `img-src https:` → `'self' data:` 축소~~ **✅ 완료(2026-07-09)** — 외부 이미지 0건 확인 후 적용
8. ~~(선택·사용자 결정 대기)~~ **ⓒ 확정·치환 완료(2026-07-09, ROADMAP U5 참조)** 도구 시그니처 색 정리 — 현재: 설계 문서 4종+성적 산출기=plan(보라), 차시=teal(역사적 경위, 팔레트 표 참조), AI 2종=ovr(인디고). 안: ⓐ 현상 유지 ⓑ 차시→plan 통일 ⓒ 계산기 2종(차시·성적)=teal로 묶어 3그룹(설계/계산기/AI) 색 인코딩 완성 — ROADMAP §3 그룹 구조와 정합해 ⓒ 권장. 결정만 나오면 Sonnet급 치환 작업

완료됨(2026-07-08, 성적 산출기 v2 구현 — Fable): `SPEC-gradecalc-v2.md` 전면 구현(gradecalc.js+style.css). ① **수행 무가공 합산** — 지필만 비율 곱, 수행 만점=반영비율(셀 max·invalid와 열 헤더 "만점 N점"을 `gcUpdatePerfMax()`가 재렌더 없이 갱신, 수행 셀 공통 클래스 `.gc-perf-input`) ② **등급 판정을 누적인원 경계로 교체** — `gcGradeCums(N)`: `GC_GRADE_TABLE` 조견표 우선(현재 N=5=[1,2,3,4,5]만) + 반올림 공식 폴백 + 단조성 보정. 석차백분율(r.pct)은 표시용으로만 유지 ③ 산출 방식 설명 `<details class="gc-explain">`(RANK.EQ/중간석차/등급 경계/모드 차이) ④ **수강자수→등급별 인원 미리보기 카드**(`.gc-preview`) — `gc:previewN`이 칩 컨테이너만 교체(포커스 보존), 판정과 같은 gcGradeCums 공유. 툴바의 인원 입력+번호 생성은 이 카드로 통합 이동(#gcFillN), `gcAutoFill`에 빈 입력 가드 추가, `.gc-fill-group` CSS 제거 ⑤ 경고문 지필(0~100)/수행(만점 초과) 분리, CSV 헤더 `수행점수(만점=반영비율)`, gc-note 문구 교체. 검증: verify 4항목 + 헤드리스 프로브 19개 시나리오 PASS(스펙 §5 8종 전부 포함), sw.js **v20**. ⚠️ 스펙 §2 표의 N=5 기대분포 "1/1/1/1/2"는 합이 6이라 N=5에서 불성립(기록 오류) — 구현은 사용자 확인 누적값 [1,2,3,4,5](분포 1/1/1/1/1)를 따름. 공식 조견표 수신 시 GC_GRADE_TABLE 보강(SPEC §6, Sonnet급).

완료됨(2026-07-08, 성적 산출기 v2 설계 — Fable): 사용자 보고 4건(①수행이 비율 곱으로 반영되는 버그 → 수행은 만점=반영비율로 **무가공 합산**, 지필만 비율 곱 ②5명 입력 시 1등급 0명 → 석차백분율 직접 판정의 구조적 결함, **누적인원 경계(조견표 lookup 우선 + 반올림 공식 폴백)** 방식으로 재설계 ③중간석차·RANK.EQ 산출식 본문 설명 ④수강자수→등급별 인원 미리보기 신설)을 **`SPEC-gradecalc-v2.md`**로 설계 완료. ⚠️ N=5의 조견표 기대값(1/1/1/1/2)이 반올림 공식과 불일치(스펙 §2 표) — 사용자의 공식 조견표 복붙 대기, 받으면 GC_GRADE_TABLE에 입력. → **구현 완료(바로 위 항목 참조).**
